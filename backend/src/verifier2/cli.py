...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--screenshot-dir", dest="screenshot_dir", default="screenshot", help="Where to save screenshots.")
    p.add_argument("--no-screenshot", dest="enable_screenshot", action="store_false", default=True, help="Disable screenshot capture.")
    p.add_argument("--download-dir", dest="download_dir", default="report", help="Where to save downloaded standard files.")
    p.add_argument("--no-download", dest="enable_download", action="store_false", default=True, help="Disable standard file download.")
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    for idx, gb_code in enumerate(gb_codes, start=1):
        ...
        try:
            ...
            if args.enable_screenshot:
                ...
                if detail_url:
                    screenshot_success, screenshot_path, screenshot_error = screenshot_detail_page(
                        detail_url=detail_url,
                        gb_number=gb_number,
                        screenshot_dir=args.screenshot_dir,
                        gb_code_for_filename=gb_code,
                    )
                    if not screenshot_success:
                        output_lines.append(f"提示：详情页截图失败（{screenshot_error}）")
                        output_lines.append("")

            # 下载标准文件
            download_path = None
            if args.enable_download and html_content:
                detail_url = parsed.get('foodmate_detail_page_url')
                download_success, download_path, download_error = download_standard_from_html(
                    html=html_content,
                    gb_number=gb_number,
    ...
    if minutes > 0:
        print(f"总运行时长：{minutes} 分 {seconds:.2f} 秒")
    else:
        print(f"总运行时长：{seconds:.2f} 秒")
