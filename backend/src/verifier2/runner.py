...
_DETAIL_URL_PATTERN = re.compile(r"https?://down\.foodmate\.net/standard/sort/\d+/\d+\.html")


def _extract_detail_urls_from_raw(raw: Optional[str], *, limit: int = 50) -> list[str]:
    """
    Extract detail-page candidate URLs from Tavily extracted raw_content.

    Intentionally small + deterministic output for debug metrics:
    - de-duplicate while preserving order
    - cap length to avoid bloating artifacts
    """
    if not isinstance(raw, str) or not raw:
        return []
    out: list[str] = []
    seen: set[str] = set()
    for m in _DETAIL_URL_PATTERN.finditer(raw):
        u = m.group(0)
        if u in seen:
            continue
        seen.add(u)
...
def run_smoke(mcp_url: str, gb_number: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    debug: dict[str, Any] = {
        ...
        "tavily_search": {},
        "tavily_extract_search_page": {
            "markdown_advanced_images": {},
            "markdown_basic": {},
            "text_basic": {},
        },
        "detail_url_chosen": None,
        "detail_url_chosen_by": None,
    ...
    if tool_name and chosen_tool:
        ...
        # --- debug: tavily_search URLs (do not affect control flow) ---
        try:
            results = (
                (((search_resp.get("body") or {}).get("result") or {}).get("structuredContent") or {}).get("results")
            )
        except Exception:
            results = None
        sort_urls: list[str] = []
        if isinstance(results, list):
            for r in results:
                u = (r or {}).get("url")
                if isinstance(u, str) and _DETAIL_URL_PATTERN.search(u):
                    sort_urls.append(u)
        debug["tavily_search"] = {
            "tool_used": tool_name,
            "first_sort_url": sort_urls[0] if sort_urls else None,
            "sort_url_candidates": sort_urls[:20],
            "num_results": len(results) if isinstance(results, list) else None,
        }
        ...
        if extract_tool:
            ...
            # Optimization: skip the markdown/advanced (+images) search-page extract call.
            # Keep all downstream logic intact by leaving `extract_resp` as None.
            raw0 = _safe_get_raw_content(extract_resp)
            urls0 = _extract_detail_urls_from_raw(raw0)
            debug["tavily_extract_search_page"]["markdown_advanced_images"] = {
                "args": extract_args,
                "raw_len": len(raw0) if isinstance(raw0, str) else None,
                "detail_url_candidates": urls0,
                "first_detail_url": urls0[0] if urls0 else None,
                "hit_any": bool(urls0),
                "skipped": True,
            }
            ...
            raw1 = _safe_get_raw_content(extract_resp_alt)
            urls1 = _extract_detail_urls_from_raw(raw1)
            debug["tavily_extract_search_page"]["markdown_basic"] = {
                "args": extract_args_alt,
                "raw_len": len(raw1) if isinstance(raw1, str) else None,
                "detail_url_candidates": urls1,
                "first_detail_url": urls1[0] if urls1 else None,
                "hit_any": bool(urls1),
            }

            extract_args_text = {"urls": [search_page_url], "format": "text", "extract_depth": "basic"}
            # Optimization: skip the text/basic extract call (empirically no contribution for detail URL discovery).
            # Keep the rest of the logic intact by leaving `extract_resp_text` as None.
            raw2 = _safe_get_raw_content(extract_resp_text)
            urls2 = _extract_detail_urls_from_raw(raw2)
            debug["tavily_extract_search_page"]["text_basic"] = {
                "args": extract_args_text,
                "raw_len": len(raw2) if isinstance(raw2, str) else None,
                "detail_url_candidates": urls2,
                "first_detail_url": urls2[0] if urls2 else None,
                "hit_any": bool(urls2),
                "skipped": True,
            }
            ...
            for candidate_name, candidate in (
                ("markdown_advanced_images", extract_resp),
                ("markdown_basic", extract_resp_alt),
                ("text_basic", extract_resp_text),
            ):
                raw = _safe_get_raw_content(candidate)
                if isinstance(raw, str):
                    m = _DETAIL_URL_PATTERN.search(raw)
                    if m:
                        detail_url = m.group(0)
                        debug["detail_url_chosen_by"] = f"tavily_extract_search_page:{candidate_name}"
                        break
                    ...
                    if isinstance(fb_results, list):
                        for i, r in enumerate(fb_results):
                            ...
                            if isinstance(u, str) and re.search(r"/standard/sort/\d+/\d+\.html$", u):
                                detail_url = u
                                debug["detail_url_chosen_by"] = f"fallback_tavily_search:index={i}"
                                break

            if detail_url:
                # Optimization: skip Tavily detail-page extraction (markdown/text).
                # We rely on direct HTML fetching/parsing later for dates/status/etc.
                #
                # IMPORTANT: do not change the detail URL acquisition logic above.
                # Keep the discovered `detail_url` available to downstream logic by
                # providing a minimal placeholder response that carries the URL.
                extract_detail_text = {
                    "body": {"result": {"structuredContent": {"results": [{"url": detail_url, "raw_content": ""}]}}}
                }
                debug["detail_url_chosen"] = detail_url
