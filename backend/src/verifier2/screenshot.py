...
def screenshot_detail_page(
    ...
    gb_code_for_filename: Optional[str] = None,
    ...
    Args:
        ...
        gb_code_for_filename: 完整 GB 编号（用于文件命名，可选）
    ...
    # 生成文件名
    # Use full GB code for filename when available to avoid collisions, e.g.:
    #   "GB 5009.33-2016" -> "gb_5009.33-2016_detail.png"
    # Fallback to the short gb_number ("33-2016") if not provided.
    source = (gb_code_for_filename or gb_number or "").strip()
    source = re.sub(r"(?i)^\s*GB(?:/T)?\s*", "", source).strip()
    source = source.replace(" ", "")
    safe_gb = source.replace("/", "-").replace("\\", "-")
